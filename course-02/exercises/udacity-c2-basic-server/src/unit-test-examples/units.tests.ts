...
describe('concat', () => {
  ...
  it('should concat "I am " by "Bao"', () => {
    const result = concat("I am ", "Bao");
    expect(result).to.equal("I am Bao");
  });

  it('should throw an error if div by empty', () => {
    expect(()=>{ concat("", "Bao") }).to.throw('strings are empty')
  });

  it('should throw an error if div by empty', () => {
    expect(()=>{ concat("I am", null) }).to.throw('strings are empty')
  });
